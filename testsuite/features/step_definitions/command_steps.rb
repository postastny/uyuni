...
Then(/^reverse resolution should work for "([^"]*)"$/) do |host|
  ...
  raise ScriptError, "reverse resolution returned #{result}, expected to see #{node.full_hostname}" unless result.include?(node.full_hostname)
end
...
Then(/^it should be possible to reach the build sources$/) do
  if $product == 'Uyuni'
    ...
    warn 'Sanity check not implemented, move resource to external network first'
  else
...
When(/^I delete these channels with spacewalk-remove-channel:$/) do |table|
  ...
  $command_output, _return_code = $server.run(channels_cmd, check_errors: false)
end

When(/^I list channels with spacewalk-remove-channel$/) do
  $command_output, return_code = $server.run('spacewalk-remove-channel -l')
  raise ScriptError, 'Unable to run spacewalk-remove-channel -l command on server' unless return_code.zero?
end
...
Then(/^spacewalk-channel should fail adding "([^"]*)"$/) do |invalid_channel|
  ...
  raise ScriptError, "#{command} should fail, but hasn't" if code.zero?
end
...
When(/^I query latest Salt changes on ubuntu system "(.*?)"$/) do |host|
  node = get_target(host)
  result, _return_code = node.run('zcat /usr/share/doc/salt-minion/changelog.Debian.gz')
  result.split("\n")[0, 15].each do |line|
    line.force_encoding('UTF-8')
    puts line
  end
...
When(/^I apply highstate on "([^"]*)"$/) do |host|
  ...
  elsif host.include?('minion') || host.include?('build_host')
    cmd = 'salt'
    extra_cmd = ''
  end
...
When(/^I kill all running spacewalk-repo-sync, excepted the ones needed to bootstrap$/) do
  do_not_kill = CommonLib.compute_list_to_leave_running
  reposync_not_running_streak = 0
  reposync_left_running_streak = 0
...
Then(/^the reposync logs should not report errors$/) do
  result, code = $server.run('grep "ERROR:" /var/log/rhn/reposync/*.log', check_errors: false)
...
When(/^I wait until the channel "([^"]*)" has been synced$/) do |channel|
  CommonLib.repeat_until_timeout(timeout: 7200, message: 'Channel not fully synced') do
...
When(/^I wait until file "([^"]*)" contains "([^"]*)" on server$/) do |file, content|
  CommonLib.repeat_until_timeout(message: "#{content} not found in file #{file}", report_result: true) do
...
Then(/^file "([^"]*)" should not contain "([^"]*)" on server$/) do |file, content|
  ...
  raise ScriptError, "'#{content}' found in file #{file}" if output != "notfound\n"
...
When(/^I extract the log files from all our active nodes$/) do
  $nodes.each do |node|
    ...
    CommonLib.extract_logs_from_node(node)
  end
end
...
Then(/^I should see "([^"]*)", "([^"]*)" and "([^"]*)" in the repo file on the "([^"]*)"$/) do |protocol, hostname, port, target|
  ...
  puts "Protocol: #{uri.scheme}  Host: #{uri.host}  Port: #{uri.port}"
  parameters_matches = (uri.scheme == protocol && uri.host == hostname && uri.port == port.to_i)
  raise ArgumentError, 'Some parameters are not as expected' unless parameters_matches
end
...
When(/^I import the GPG keys for "([^"]*)"$/) do |host|
  node = get_target(host)
  gpg_keys = get_gpg_keys(node, $server)
...
When(/^I synchronize the tftp configuration on the proxy with the server$/) do
  ...
  raise(ScriptError, 'cobbler sync failed') if out.include?('Push failed')
end
...
Then(/^I wait until mgr-sync refresh is finished$/) do
  ...
  CommonLib.repeat_until_timeout(timeout: 1800, message: "'mgr-sync refresh' did not finish") do
...
Then(/^service or socket "([^"]*)" is enabled on "([^"]*)"$/) do |name, host|
  ...
  raise(StandardError, 'Service or socket not enabled') if (output_service != 'enabled') && (output_socket != 'enabled')
end
...
Then(/^service or socket "([^"]*)" is active on "([^"]*)"$/) do |name, host|
  ...
  output_socket = output_socket.split(/\n+/)[-1]
  raise(StandardError, 'Service or socket not active') if (output_service != 'active') && (output_socket != 'active')
end
...
When(/^I wait at most (\d+) seconds until file "([^"]*)" exists on "([^"]*)"$/) do |seconds, file, host|
  node = get_target(host)
  CommonLib.repeat_until_timeout(timeout: seconds.to_i) do
...
When(/^I wait until file "(.*)" exists on server$/) do |file|
  CommonLib.repeat_until_timeout do
...
Then(/^I wait and check that "([^"]*)" has rebooted$/) do |host|
  ...
  check_restart(system_name, reboot_timeout)
end
...
When(/^I wait for the OpenSCAP audit to finish$/) do
  ...
  @sle_id = CommonLib.retrieve_server_id($minion.full_hostname)
  @client_api = XMLRPC::Client.new2("http://#{host}/rpc/api")
  @sid = @client_api.call('auth.login', 'admin', 'admin')
  begin
    CommonLib.repeat_until_timeout(message: 'process did not complete') do
...
When(/^I register this client for SSH push via tunnel$/) do
  ...
  script = "spawn spacewalk-ssh-push-init --client #{$client.ip} --register #{bootstrap} --tunnel\n" \
           ...
           "  }\n" \
           "}\n"
  path = CommonLib.generate_temp_file('push-registration.exp', script)
...
When(/^I (enable|disable) Ubuntu "([^"]*)" repository on "([^"]*)"$/) do |action, repo, host|
  ...
  raise ArgumentError, "#{node.hostname} is not a Ubuntu host." unless os_family =~ /^ubuntu/
...
When(/^I (install|remove) OpenSCAP dependencies (on|from) "([^"]*)"$/) do |action, where, host|
  ...
  when /^ubuntu/
    ...
    raise(ArgumentError, "The node #{node.hostname} has not a supported OS Family (#{os_family})")
  end
...
When(/^I wait until the package "(.*?)" has been cached on this "(.*?)"$/) do |pkg_name, host|
  ...
  CommonLib.repeat_until_timeout(message: "Package #{pkg_name} was not cached") do
...
When(/^I create the bootstrap repository for "([^"]*)" on the server$/) do |host|
  base_channel = BASE_CHANNEL_BY_CLIENT[host.to_sym]
  channel = CHANNEL_TO_SYNC_BY_BASE_CHANNEL[base_channel.to_sym]
  parent_channel = PARENT_CHANNEL_TO_SYNC_BY_BASE_CHANNEL[base_channel.to_sym]
  cmd =
...
When(/^I set up the private network on the terminals$/) do
  ...
  # PXE boot minion
  step %(I restart the network on the PXE boot minion) if $pxeboot_mac
end

Then(/^terminal "([^"]*)" should have got a retail network IP address$/) do |host|
  node = get_target(host)
  output, return_code = node.run('ip -4 address show eth1')
  raise ScriptError, "Terminal #{host} did not get an address on eth1: #{output}" unless return_code.zero? && output.include?(net_prefix)
end
...
Then(/^name resolution should work on terminal "([^"]*)"$/) do |host|
  ...
  %w[proxy.example.org dns.google.com].each do |dest|
    ...
    raise(ScriptError, "Direct name resolution of #{dest} on terminal #{host} doesn't work: #{output}") unless return_code.zero?
  ...
  [client, '8.8.8.8'].each do |dest|
    output, return_code = node.run("host #{dest}", check_errors: false)
    raise ScriptError, "Reverse name resolution of #{dest} on terminal #{host} doesn't work: #{output}" unless return_code.zero?
...
When(/^I configure the proxy$/) do
  ...
  settings = "RHN_PARENT=#{$server.ip}\n" \
             ...
             "RHN_USER=admin\n" \
             "ACTIVATE_SLP=y\n"
  path = CommonLib.generate_temp_file('config-answers.txt', settings)
...
Then(/^The metadata buildtime from package "(.*?)" match the one in the rpm on "(.*?)"$/) do |pkg, host|
  ...
  raise ScriptError, "Command failed: #{cmd}" unless return_code.zero?

  cmd = "rpm -q --qf '%{BUILDTIME}' #{pkg}"
  rpm_buildtime, return_code = node.run(cmd)
  raise ScriptError, "Command failed: #{cmd}" unless return_code.zero?
  raise ScriptError, "Wrong buildtime in metadata: #{metadata_buildtime} != #{rpm_buildtime}" unless metadata_buildtime == rpm_buildtime
end
...
When(/^I create "([^"]*)" virtual machine on "([^"]*)"$/) do |vm_name, host|
  ...
  raise ScriptError, '/var/testsuite-data/disk-image-template.qcow2 not found' unless file_exists?(
    node,
    '/var/testsuite-data/disk-image-template.qcow2'
  )
  ...
  node.run(
    "virt-install --name #{vm_name} --memory 512 --vcpus 1 --disk path=#{disk_path} "\
...
When(/^I create ([^ ]*) virtual network on "([^"]*)"$/) do |net_name, host|
  ...
  networks = { 'test-net0': { bridge: 'virbr0', subnet: 124 }, 'test-net1': { bridge: 'virbr1', subnet: 126 } }
...
Then(/^I should see "([^"]*)" virtual machine (shut off|running|paused) on "([^"]*)"$/) do |vm, state, host|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual machine on #{host} never reached state #{state}") do
...
When(/^I wait until virtual machine "([^"]*)" on "([^"]*)" is started$/) do |vm, host|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual machine on #{host} OS failed did not come up yet") do
...
Then(/^I should not see a "([^"]*)" virtual machine on "([^"]*)"$/) do |vm, host|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual machine on #{host} still exists") do
...
Then(/^"([^"]*)" virtual machine on "([^"]*)" should have ([0-9]*)MB memory and ([0-9]*) vcpus$/) do |vm, host, mem, vcpu|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual machine on #{host} never got #{mem}MB memory and #{vcpu} vcpus") do
    output, _code = node.run("virsh dumpxml #{vm}")
    has_memory = output.include?("<memory unit='KiB'>#{mem.to_i * 1024}</memory>")
...
Then(/^"([^"]*)" virtual machine on "([^"]*)" should have ([a-z]*) graphics device$/) do |vm, host, type|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual machine on #{host} never got #{type} graphics device") do
    output, _code = node.run("virsh dumpxml #{vm}")
    check_nographics = type == 'no' and !output.include?('<graphics')
...
Then(/^"([^"]*)" virtual machine on "([^"]*)" should have ([0-9]*) NIC using "([^"]*)" network$/) do |vm, host, count, net|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual machine on #{host} never got #{count} network interface using #{net}") do
    output, _code = node.run("virsh dumpxml #{vm}")
...
Then(/^"([^"]*)" virtual machine on "([^"]*)" should have a NIC with ([0-9a-zA-Z:]*) MAC address$/) do |vm, host, mac|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual machine on #{host} never got a network interface with #{mac} MAC address") do
    output, _code = node.run("virsh dumpxml #{vm}")
...
Then(/^"([^"]*)" virtual machine on "([^"]*)" should have a "([^"]*)" ([^ ]*) disk$/) do |vm, host, path, bus|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual machine on #{host} never got a #{path} #{bus} disk") do
    output, _code = node.run("virsh dumpxml #{vm}")
...
Then(/^"([^"]*)" virtual machine on "([^"]*)" should have a "([^"]*)" ([^ ]+) disk from pool "([^"]*)"$/) do |vm, host, vol, bus, pool|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual machine on #{host} never got a #{vol} #{bus} disk from pool #{pool}") do
    output, _code = node.run("virsh dumpxml #{vm}")
    tree = Nokogiri::XML(output)
    disks =
      tree.xpath('//disk').select do |x|
...
Then(/^"([^"]*)" virtual machine on "([^"]*)" should have (no|a) ([^ ]*) ?cdrom$/) do |vm, host, presence, bus|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual machine on #{host} #{presence == 'a' ? 'never got' : 'still has'} a #{bus} cdrom") do
    output, _code = node.run("virsh dumpxml #{vm}")
    tree = Nokogiri::XML(output)
...
Then(/^"([^"]*)" virtual machine on "([^"]*)" should have "([^"]*)" attached to a cdrom$/) do |vm, host, path|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual machine on #{host} never got a #{path} attached to cdrom") do
    output, _code = node.run("virsh dumpxml #{vm}")
    tree = Nokogiri::XML(output)
    disks = tree.xpath('//disk')
    disk_index = disks.find_index { |x| x.attribute('device').to_s == 'cdrom' }
...
Then(/^"([^"]*)" virtual machine on "([^"]*)" should (not stop|stop) on reboot((?: at next start)?)$/) do |vm, host, stop, next_start|
  ...
  break if (on_reboot == 'destroy' && stop == 'stop') || (on_reboot == 'restart' && stop == 'not stop')
...
Then(/^"([^"]*)" virtual machine on "([^"]*)" should be UEFI enabled$/) do |vm, host|
  ...
  raise(ScriptError, 'No loader and nvram set: not UEFI enabled') unless has_loader && has_nvram
end
...
When(/^I delete all "([^"]*)" volumes from "([^"]*)" pool on "([^"]*)" without error control$/) do |volumes, pool, host|
  node = get_target(host)
  output, _code = node.run(
    "virsh vol-list #{pool} | sed -n -e 's/^[[:space:]]*\([^[:space:]]\+\).*$/\1/;/#{volumes}/p'", check_errors: false
  )
...
Then(/^I should not see a "([^"]*)" virtual network on "([^"]*)"$/) do |vm, host|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual network on #{host} still exists") do
...
Then(/^I should see a "([^"]*)" virtual network on "([^"]*)"$/) do |vm, host|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{vm} virtual network on #{host} still doesn't exist") do
    _output, code = node.run("virsh net-info #{vm}", check_errors: false)
    break if code.zero?
...
Then(/^"([^"]*)" virtual network on "([^"]*)" should have "([^"]*)" IPv4 address with ([0-9]+) prefix$/) do |net, host, ip, prefix|
  node = get_target(host)
  CommonLib.repeat_until_timeout(message: "#{net} virtual net on #{host} never got #{ip}/#{prefix} IPv4 address") do
...
Then(/^I wait until refresh package list on "(.*?)" is finished$/) do |client|
  # spacecmd uses timestamps with precision to minutes only
  round_minute = 60
  long_wait_delay = 600
  ...
  refreshes.split.each do |refresh_id|
    ...
    # Filter refreshes for specific system
    refresh_result, = $server.run("spacecmd -u admin -p admin schedule_details #{refresh_id}")
  ...
  CommonLib.repeat_until_timeout(timeout: long_wait_delay, message: "'refresh package list' did not finish") do
    result, _code = $server.run(cmd, check_errors: false)
    sleep 1
...
When(/^I wait until package "([^"]*)" is installed on "([^"]*)" via spacecmd$/) do |pkg, client|
  ...
  $server.run('spacecmd -u admin -p admin clear_caches')
  command = "spacecmd -u admin -p admin system_listinstalledpackages #{node}"
  CommonLib.repeat_until_timeout(timeout: 600, message: "package #{pkg} is not installed yet") do
    result, _code = $server.run(command, check_errors: false)
...
When(/^I wait until package "([^"]*)" is removed from "([^"]*)" via spacecmd$/) do |pkg, client|
  ...
  $server.run('spacecmd -u admin -p admin clear_caches')
  command = "spacecmd -u admin -p admin system_listinstalledpackages #{node}"
  CommonLib.repeat_until_timeout(timeout: 600, message: "package #{pkg} is still present") do
    result, _code = $server.run(command, check_errors: false)
    sleep 1
...
When(/^I remove all the DHCP hosts created by retail_yaml$/) do
  terminals = read_terminals_from_yaml
  terminals.each do |terminal|
    raise ScriptError unless find(:xpath, "//*[@value='#{terminal}']/../../../..//*[@title='Remove item']").click
  end
end

When(/^I remove the bind zones created by retail_yaml$/) do
  domain = read_branch_prefix_from_yaml
  raise ScriptError unless find(
    :xpath,
    "//*[text()='Configured Zones']/../..//*[@value='#{domain}']/../../../..//*[@title='Remove item']"
  ).click

  raise ScriptError unless find(
    :xpath,
    "//*[text()='Available Zones']/../..//*[@value='#{domain}' and @name='Name']/../../../..//i[@title='Remove item']"
  ).click
end
...
Then(/^the "([^"]*)" on "([^"]*)" grains does not exist$/) do |key, client|
  ...
  raise ScriptError if code.zero?
end
...
When(/^I (enable|disable) the necessary repositories before installing Prometheus exporters on this "([^"]*)"((?: without error control)?)$/) do |action, host, error_control|
  ...
  node = get_target(host)
  _os_version, os_family = get_os_version(node)
  step %(I #{action} repository "tools_additional_repo" on this "#{host}"#{error_control}) if (os_family =~ /^opensuse/ || os_family =~ /^sles/) && $product != 'Uyuni'
end
...
When(/^I copy autoinstall mocked files on server$/) do
  ...
  return_codes = []
  return_codes << file_inject(
    $server,
    "#{base_dir}fedora12/vmlinuz",
    "#{source_dir}Fedora_12_i386/images/pxeboot/vmlinuz"
  )
  return_codes << file_inject(
    $server,
    "#{base_dir}fedora12/initrd.img",
    "#{source_dir}Fedora_12_i386/images/pxeboot/initrd.img"
  )
  return_codes << file_inject($server, "#{base_dir}mock/empty.xml", "#{source_dir}mock/empty.xml")
  return_codes << file_inject(
    $server,
    "#{base_dir}sles15sp2/initrd",
    "#{source_dir}SLES15-SP2-x86_64/DVD1/boot/x86_64/loader/initrd"
  )
  return_codes << file_inject(
    $server,
    "#{base_dir}sles15sp2/linux",
    "#{source_dir}SLES15-SP2-x86_64/DVD1/boot/x86_64/loader/linux"
  )
...
When(/^I copy unset package file on server$/) do
  ...
  return_code = file_inject(
    $server,
    "#{base_dir}subscription-tools-1.0-0.noarch.rpm",
...
Then(/^"(.*?)" folder on server is ISS v2 export directory$/) do |folder|
  raise(ScriptError, "Folder #{folder} not found") unless file_exists?($server, "#{folder}/sql_statements.sql")
end

Then(/^Export folder "(.*?)" doesn't exists on server$/) do |folder|
  raise(ScriptError, 'Folder exists') if folder_exists?($server, folder)
end
